...
- 상단 사용자 상태는 로그인 전 `로그인`, 로그인 후 `이름님`으로 표시됩니다.
- 로그인 후 상단 사용자 버튼을 누르면 사용자 정보 화면이 열립니다.
- 사용자 정보 화면에서는 이름/전화번호 조회, 현재 비밀번호 확인 기반 비밀번호 변경, 로그아웃이 가능합니다.
- 찜 기능은 MySQL `favorites` 테이블과 연결되었습니다.
- 찜 목록은 DB에서 불러오며, 전체/상의/하의/모자/신발 탭과 세로 스크롤 카드 목록으로 표시됩니다.
- 찜 목록 카드에는 이미지, 종류, 의상 이름, 착용 버튼, 삭제 버튼이 표시됩니다.
...
- `GET /api/users/<user_id>`: 사용자 이름/전화번호 조회
- `POST /api/users/<user_id>/password`: 현재 비밀번호 확인 후 비밀번호 변경
- `GET /api/users/<user_id>/favorites`: 사용자 찜 목록 조회
- `POST /api/favorites/toggle`: 찜 토글
- `POST /api/favorites/remove`: 찜 삭제
...
- 로그인/회원가입/사용자 정보 UI 디자인을 아직 더 다듬어야 합니다. 현재는 테스트 가능한 입력/버튼/API 연결이 우선입니다.
